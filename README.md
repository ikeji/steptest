...
- **アクションメニュー**: 3D ビュー右上のコンテキストメニュー。未選択時は形状 (直方体/円柱/球、および定義済み関数の呼び出し) を追加でき、形状を選択中は選択している形状を変形 (移動/回転X・Y・Z/拡大縮小/フィレット/面取り) ブロックで包む。追加・ラップ後は対象が自動選択されるので、そのままギズモや連続ラップで編集を続けられる。
- **関数による抽象化**: 選択中の形状をメニューの「関数にまとめる」で関数定義 (`関数〜返す`) に切り出し、元の場所は呼び出しブロックに置き換わる。定義した関数は未選択メニューの「関数「名前」」ボタンで何個でも配置できる。Blockly 標準の関数ブロック (引数付き) もツールボックスの「関数」カテゴリから使える。関数定義の中のブロックは引数の値が決まらないため選択してもプレビューにはならず、全体表示のまま。
- **3D ビューでの選択**: ビュー内の形状をクリックするとそれを作ったブロック (「表示する」に繋がっている先頭ブロック) が選択され、空きスペースをクリックすると選択解除。生成コードに形状→ブロックIDの対応 (`shapeIds`) を埋め込み、クリック時にレイキャストで逆引きしている。
- **複数選択とブール演算**: Shift+クリックで形状を2つまで選択 (1つ目=青、2つ目=橙のハイライト、全体表示のまま)。2つ選ぶとメニューから合体/けずる (青−橙)/共通部分でひとつのブール演算ブロックにまとめられる。空いた「表示する」は自動で片付く。
- **移動ギズモ・回転ダイヤル**: 移動ブロックを選択すると XYZ 矢印 (TransformControls)、回転ブロックを選択すると自作の回転ダイヤル (全周リング+30°目盛り+0°基準線+現在角度の針、左上に角度表示) が 3D ビューに出て、ドラッグでブロックの数値を直接編集できる (移動 0.1 / 角度 1° 単位に丸め)。ダイヤルはノブのようにポインタ方向へ追従し、サイズは形状に合わせて自動調整。数値の入力に式が入っている場合はギズモは出ない。
...
- Blockly 標準の数値・演算・くり返し・変数・関数ブロックも使えるので、ループでの繰り返し配置や関数による部品の再利用が可能
